...
cores=4
memory=5120
hosts="gpu04 gpu05 gpu06 gpu07 gpu08 gpu09 gpu10 gpu11 gpu12"
...
#                         args="--weight_file=${weightsFile}  --faulty --injection --layer=${layer} --bit=${bit} --${loc} --quantize --quant-feats=${qbit} --quant-wts=${qbit} --quant-accum=${qbitsAcc}"
#                         bsub -q gpu -m "${hosts}" -n ${cores} -W 01:40 -R "rusage[mem=${memory},ngpus_excl_p=1]select[ncc>=3.0]" -R "span[hosts=1]" -J resnet50_int${qbit}_faulty_${layer}_bit_${bit}_loc_${loc}_iter_${iter} -o ${fidPrefix}.out -e ${fidPrefix}.err ./ghpcc_job.sh ${experiment} ${model} ${cores} "${args}" ${fidPrefix}
