...
class QFIConv2d(FIConv2d):
    
    def __init__(self, fi, id, name, in_channels, out_channels, kernel_size, stride=1, 
                 padding=0, dilation=1, groups=1, padding_mode='zeros', num_bits_acts=8, 
                 num_bits_params=8, num_bits_accum=32, mode=LinearQuantMode.SYMMETRIC, 
                 clip_acts=False, per_channel_wts=False, weight=None, bias=None):

        super(QFIConv2d, self).__init__(fi, id, name, in_channels, out_channels, kernel_size, 
    ...
    def forward(self, inputs):
        ...
        accum = super(QFIConv2d, self).forward(input_q)
    ...
    @staticmethod
    def from_pytorch_impl(fi, id, name, conv2d: nn.Conv2d):
        return QFIConv2d(fi, id, name, conv2d.in_channels, conv2d.out_channels, conv2d.kernel_size, 
                                    conv2d.stride, conv2d.padding, conv2d.dilation, conv2d.groups, 
                                    conv2d.padding_mode, fi.quantizationBitWeights, fi.quantizationBitFeatures, 
                                    fi.quantizationBitAccum, fi.quantizationType, fi.quantizationClip, 
                                    fi.quantizationChannel, conv2d.weight, conv2d.bias)
